...
class SR2QualityAnalyzer:
    ...
    def plot_quality_trend(self, df, title="Detector Quality Trend", save_prefix=""):
        # <--- UPDATED: Wider figure for better visibility (from 16,12 to 20,12)
        fig = plt.figure(figsize=(20, 12)) 
        
        # Added wspace=0.02 to shrink the horizontal gap between the plots and the colorbar
        gs = gridspec.GridSpec(2, 2, width_ratios=[20, 1], hspace=0.3, wspace=0.02)
        ...
        cbar.outline.set_linewidth(3.0)   # <--- UPDATED: Thicker border for colorbar
        cbar.outline.set_edgecolor('black') # <--- UPDATED: Black border for colorbar
        
        # <--- UPDATED: Ensure all subplot spines have thick black borders
    ...
    def plot_run_diagnostic(self, df, run_id, save_prefix=""):
        ...
        # <--- UPDATED: Wider diagnostic plot (from 12,7 to 16,7)
        ...
        ax.axvline(0, color='black', linewidth=3.0) # <--- UPDATED: Thicker center baseline
        ax.axvline(3, color='black', linestyle='--', linewidth=1.5, alpha=0.8) # <--- UPDATED: Black reference lines
        ...
        # <--- UPDATED: Ensure thick black borders for the diagnostic plot
        for spine in ax.spines.values():
            spine.set_linewidth(3.0)
            spine.set_color('black')
...
if __name__ == "__main__":
    ...
    if args.plot or args.inspect_id:
